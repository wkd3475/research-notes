...
OpenSearch 기본 기능을 논문 형태로 자세히 소개해 달라고 요청했고, 이후 아키텍처를 그림으로 보강해 계층·요청·쓰기 경로를 보기 쉽게 해 달라고 했다.
...
flowchart TB
  subgraph Cluster["OpenSearch 클러스터"]
    direction TB
    CM["Cluster manager<br/>클러스터 상태 · 할당"]
    subgraph Nodes["노드"]
      direction LR
...
```mermaid
flowchart LR
  D1["문서<br/>{ name, tags, price }"]
  D2["문서<br/>{ name, tags, price }"]
  IDX["인덱스: products<br/>settings + mapping"]
  D1 --> IDX
  D2 --> IDX
  IDX --> T["text: name<br/>분석 · BM25"]
  IDX --> K["keyword: tags<br/>exact · 집계"]
  IDX --> N["numeric: price<br/>range · 정렬 · 집계"]
```
...
flowchart TB
  F["필드: product_name"]
  F --> TXT["text<br/>전문 검색"]
  F --> RAW["keyword 서브필드 .raw<br/>필터 · 정렬 · 집계"]
```
...
```mermaid
flowchart TB
  IDX["인덱스: orders<br/>primary 3 · replica 1씩"]
...
각 샤드는 그 자체로 불변 segment들의 Lucene 인덱스다.
...
```mermaid
flowchart LR
  Raw["원문"] --> CF["Character filters"]
  CF --> TK["Tokenizer"]
  TK --> TF["Token filters<br/>소문자 · stop · stem…"]
...
flowchart TB
  ...
  Docs --> Inv
  Q["질의: beauty"] --> Inv
  Inv --> Hits["히트: Doc 1, Doc 2<br/>BM25 순위"]
```
...
```mermaid
flowchart LR
  W["Index / bulk 요청"] --> Prim["Primary shard"]
  Prim --> TL["Translog<br/>fsync → durable ack"]
  Prim --> Buf["Lucene 메모리 버퍼"]
  Prim --> Rep["Replica shards"]
  Buf -->|"refresh"| Seg["새 segment<br/>검색 가능"]
  Seg -->|"flush fsync"| Disk["Durable Lucene 파일"]
  Disk --> TL2["Translog 정리 가능"]
  Seg -->|"merge"| Big["더 큰 segment"]
```
...
flowchart TB
  Bool["bool query"]
  Bool --> Must["must<br/>필수 · 점수"]
  Bool --> Filter["filter<br/>필수 · 점수 없음 · 캐시"]
  Bool --> Should["should<br/>선택 가산"]
  Bool --> Not["must_not<br/>제외"]
```
...
```mermaid
flowchart TB
  Q["선택적 쿼리<br/>문서 집합 축소"] --> Buckets["Bucket agg<br/>예: terms(category)"]
...
flowchart TB
  ...
  Lucene["Lucene segments<br/>역색인 · doc values"]

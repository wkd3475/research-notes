...
**학생:** `scylla.yaml` 맞춰 두고 `systemctl start scylla-server` 쳤어요. 맨 처음에 뭐부터 하나요?

**선생님:** **seed 노드**부터 붙는다. `seeds`(Cassandra는 `seed_provider`)에 적어 둔 IP다. 리더도, 쓰기 조율자도, 데이터 담당 특별 노드도 아니다. **아직 링을 모를 때 gossip을 걸 첫 접점**일 뿐이다.

**학생:** seed가 전부 죽으면 끝장인가요?

**선생님:** **처음 조인하는 노드**는 맞다. gossip을 못 건다. **이미 클러스터에 있던 노드**는 다르다. 기본적으로 예전에 gossip한 peer를 기억해서 seed 없이도 다시 붙을 수 있다. seed는 **첫 기동**에서 특히 중요하다.

**학생:** 조인하는 노드를 seed에 넣으면?

**선생님:** **안 된다.** 문서에도 나온다. **seed 노드는 bootstrap을 할 수 없다.** 살아 있는 기존 노드만 `seeds`에 넣고, 조인 중인 노드 자신은 넣지 않는다. 모든 노드를 seed로 만들지도 말자. gossip 부담만 늘고 이득은 없다. **DC당 seed 2~3개**, **모든 노드가 같은 목록**을 쓰는 게 원칙이다.

**학생:** 멀티 DC면요?

**선생님:** **DC마다 seed를 최소 하나** 넣어라. replication group 단위다. 없으면 다른 DC에만 의존해서 토폴로지를 배워야 해서 느리고 불안하다.
...
**선생님:** **Gossip**이 돈다. P2P 전염(에피데믹) 방식이다. 대략 1초마다 노드가 peer 몇 개(최대 ~3)와 상태를 바꾸고, 버전이 높은 정보가 이긴다. 조인 노드가 배우는 것:

- 클러스터에 누가 있는지(IP)
- 누가 up/down인지
- **각 노드의 token** (`application_state::TOKENS`)

**학생:** discovery가 한 번이면 끝인가요?

**선생님:** 아니다. seed가 **첫 스냅샷**을 주고, 이후엔 다른 노드와 똑같이 gossip에 참여한다. 결국 모든 노드가 모든 노드를 안다.

**학생:** seed 목록을 노드마다 다르게 쓰면?

**선생님:** gossip이 갈라지거나 split-brain 위험이 생긴다. **첫 부팅**에서 특히 치명적이다. 이후엔 기억한 peer가 도와주지만, 운영에서는 여전히 **동일한 seed 목록**을 표준으로 맞춘다.
...
**학생:** 1탄에서 vnode면 `initial_token` 수동이 필요 없다고 했는데, 실제로 token은 어떻게 정해지나요?

**선생님:** 설정의 `num_tokens`를 읽는다(Scylla/Cassandra 기본 **256**). bootstrap 때 순서는 이렇다.
...
**학생:** 더 센 머신에 데이터를 더 실으려면?

**선생님:** token 수를 늘린다. 작은 노드 256, 큰 노드 512처럼. `num_tokens`가 하드웨어 비례 조절기다.

**학생:** 두 노드가 같은 token을 고르면?

**선생님:** 조인할 때 기존 token을 먼저 보고 중복은 스킵한다.

**학생:** 무작위 말고 더 똑똑한 방법은?

**선생님:** Cassandra **3.0+**에서 JVM 옵션 `-Dcassandra.allocate_tokens_for_keyspace=<keyspace>` — 그 keyspace의 기존 vnode **부하**를 보고 token을 고른다. token 수를 줄여도 균형이 나을 수 있다. 기본은 여전히 무작위다.

**학생:** `initial_token`은 언제 쓰나요?

**선생님:** `cassandra.yaml`에 쉼표로 나열하면 자동 할당을 건너뛴다. 외부 도구로 token을 짜거나, **예전 token으로 노드를 복구**할 때다. Scylla는 실무에서 **vnode 전용**이고, 레거시 single-token에서만 `initial_token`이 `num_tokens`를 덮어쓴다.
...
**학생:** token 잡았으면 바로 트래픽 받나요?

**선생님:** 아직이다. **Bootstrap**은 사람들이 한 덩어리로 부르지만 두 단계다.
...
**선생님:** 새 구간마다 **현재 replica**에서 stream한다. 기본은 구간별 **primary replica** — 클러스터 상태와 일관되게 맞추려는 선택이다. 필요한 replica가 down이면 bootstrap이 **실패**한다. `-Dcassandra.consistent.rangemovement=false`로 우회할 수 있지만 데이터가 빠질 수 있어서 위험하다.
...
**선생님:** Cassandra **2.2+**는 `nodetool bootstrap resume`, 또는 **재시작**만으로 이어지는 경우가 많다. 완전 초기화는 `-Dcassandra.reset_bootstrap_progress=true`. 그보다 낮은 버전은 데이터를 지우고 다시 bootstrap한다.

**학생:** streaming 자체를 건너뛸 수 있나요?

**선생님:** `auto_bootstrap: false`면 데이터 복사 없이 링에만 들어간다. **백업 복구**, **새 DC**처럼 데이터를 다른 경로로 넣을 때다. 기본값은 `true`(yaml에 안 보여도 켜져 있음). 일반 scale-out에서 함부로 끄지 말 것.

---

### 5막 — UN 됐다. 그런데 cleanup 함정

**학생:** `nodetool status`가 **UN**이면 끝인가요?

**선생님:** 거의 다 왔다. **기존 노드 전부에 `nodetool cleanup`**을 돌려라. 새 노드에는 하지 않는다. 구간이 옮겨가도 Cassandra/Scylla는 **더 이상 안 가진 데이터를 자동 삭제하지 않는다** — 안전장치다. cleanup 안 하면 디스크 부담에 남고, 나중에 꼬일 수 있다.

**학생:** cleanup 부담이 큰데 미뤄도 되나요?

**선생님:** **한가한 시간**으로 미룰 수는 있다. 다만 **decommission/removal 전에** 끝내야 한다. 안 그러면 **data resurrection** 위험이 있다. 노드를 여러 대 추가할 때 팁:

...
:::chat gon Gon
근데 신규 DC 추가하는 상황에서도 자동으로 streaming 해서 복제해오나?
:::

:::chat teacher 선생님
아니 — 신규 DC는 같은 DC scale-out이랑 달라. bootstrap streaming은 **같은 DC의 replica**에서만 받으니까, 신규 DC는 처음에 로컬에 데이터가 없어서 조인해도 대량 복제가 안 일어나.

Scylla 절차는 이렇게 나뉘어:

1. 신규 DC 노드 기동 → 링 조인 (UN까지)
2. `ALTER KEYSPACE`로 신규 DC replication factor 설정
3. 신규 DC **각 노드**에서 `nodetool rebuild <기존_DC>` **수동 실행**
4. 클러스터 repair

`auto_bootstrap`을 굳이 `false`로 둘 필요는 없어 — cross-DC 데이터는 bootstrap으로 안 들어오거든. **데이터 채우기는 rebuild가 담당**이야.
:::

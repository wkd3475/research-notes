...
:::chat gon Gon
3막 anti-pattern 표에 collection이 있는데, collection 개념을 더 설명해 주세요.
:::

:::chat teacher 선생님
CQL/Scylla에서 **collection**은 **한 row의 한 column(한 셀)** 안에 여러 값을 넣는 **복합 타입**이다. 별도 row가 아니다. 종류는 세 가지 ([CQL 문서](https://docs.scylladb.com/manual/stable/cql/types.html#collections)).

| 타입 | 의미 | 예시 |
|------|------|------|
| **list** | 순서 있는 값 (중복 가능) | `scores list<int>` → `[3, 9, 4]` |
| **set** | 정렬된 **유니크** 값 | `tags set<text>` → `{'kitten','cat'}` |
| **map** | key → value (key 유니크) | `favs map<text,text>` → `{'fruit':'Banana'}` |
...
row
  ├─ name: "Alice"
  └─ tags: set<text>   ← column 하나 = 저장 단위 하나
```

원소 하나만 읽어도 **collection 전체**를 읽는 경로가 된다(셀 안 paging 없음). 전화번호 몇 개·라벨 몇 개 같은 **소량 비정규화**용이지, 무한히 늘 스트림용이 아니다.
:::

:::chat student AI 학생
frozen이랑 non-frozen collection 차이는요?
:::

:::chat teacher 선생님
| | **non-frozen** (기본) | **frozen** |
|--|------------------------|------------|
| 수정 | 원소 추가/삭제 (`tags = tags + {'vip'}`) | **값 통째로만** 교체 |
| 용도 | 가끔 바뀌는 작은 집합 | 한 번에 쓰는 blob, PK, 중첩 collection |
| append 비용 | **읽기 → merge → 쓰기**, collection 크기에 비례 | 덮어쓰기 위주 |
...
PK나 collection 안의 collection은 반드시 `frozen<...>`.
:::

:::chat teacher 선생님
**맞는 경우:** 개수 bounded, row와 같이 읽는 메타 — 사용자 태그, 전화번호 몇 개, 작은 옵션 map.

**anti-pattern** ([modeling mistakes](https://www.scylladb.com/2023/09/11/nosql-data-modeling-mistakes-that-hurt-performance/)) — 이벤트를 map 한 칸에 계속 쌓기:

```sql
-- 나쁜 예: 센서 이벤트를 map 셀에 append
...
append마다 collection 전체 read → merge(sorted vector) → write. ingest당 **O(n)** → throughput↓, p99↑.

**list보다 set** — list 인덱스 수정은 read-before-write, 재시도가 idempotent 아닐 수 있음 ([CQL lists](https://docs.scylladb.com/manual/stable/cql/types.html#lists)).

**수정안 — clustering key row:**
...
append = **새 row INSERT**(LSM에 맞음). 구간 읽기 = clustering query. 파티션 삭제 = `DELETE WHERE sensor_id = ?`.

| collection은… | collection 아님… |
|---------------|----------------|
| 한 row 안 **작은** set/map | 시계열·로그·메시지 스트림 |
| 부모 row와 같이 읽는 메타 | 한 셀에 무한 append |
| 가능하면 **set** > **list** | 셀 하나에 원소만 계속 증가 |
:::
...
:::quiz
**Q6.** CQL collection은 언제 맞고, clustering key row는 언제 써야 하나?
---
**collection:** 한 row 안 소량·bounded(태그, 전화번호 몇 개), 부모와 같이 읽음. **row로 쪼개기:** 무한 append(IoT·로그) — non-frozen collection append는 쓰기마다 O(n) merge; `(pk, clustering_key)` INSERT가 LSM에 맞고 거대 셀을 피함.
:::
